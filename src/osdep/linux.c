...
/* Mehdi: Add the option to set txpower */
static int linux_set_txpower(struct wif *wi, int txpower)
{
	struct priv_linux *dev = wi_priv(wi);
	char s[32];
	int fd, pid, status;
	...
	switch (dev->drivertype) {
	case DT_ZD1211RW:
		snprintf(s, sizeof(s) - 1, "%d", txpower);
		if ((pid = fork()) == 0) {
			close(0); close(1); close(2);
			if (chdir( "/" ) == -1)
				perror("chdir");
	...
	if (iw_set_ext(fd, wi_get_ifname(wi), SIOCSIWTXPOW, &wrq) < 0) {
		return -1;
	}
...
static int do_linux_open(struct wif *wi, char *iface)
{
	...
	if (!iwpriv) {
		fprintf(stderr, "Can't find wireless tools, exiting. Under Openwrt, if you have a PRISM chipset, you need wl. This can be installed by typing: ipkg install wl. Likewise, Atheros/MadWIFI users need wlanconfig. This *should* be in either kmod-madwifi or wireless-tools.\n");
		goto close_in;
	}
